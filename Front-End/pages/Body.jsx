...
const Body = () => {
    ...
    const fetch = async () => {
      try {
        const response = await axios.get(
          `${window.location.origin}/app/task/get`, { headers }
        )
  ...
  let deleteItem = async (id) => {
    try {
      ...
      const response = await axios.delete(`${window.location.origin}/app/task/delete/${id}`, { headers })
  ...
  let finishItem = async (id) => {
    try {
      event.preventDefault()
      const response = await axios.put(`${window.location.origin}/app/task/finish/${id}`, {}, { headers })
      alert(response.data.message)
    } catch (error) {
      alert(error.response.data.message)
    }
